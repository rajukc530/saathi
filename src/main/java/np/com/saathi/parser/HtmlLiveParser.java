		...
		try {
			...
			for (Element ptext : date_time) {
				if (ptext.text() != null && ptext.text().contains("As of")) {
					dateTime += ptext.text().substring(6,25);
					System.out.println("dateTime:"+dateTime);
					break;
				}
			}

			Elements table_live = doc.select("table.table-condensed.nepse-live");
			Elements tr_live = table_live.select("tr");
			long cnt = 1;
			for (Element tr : tr_live) {
			//	System.out.println("tr:"+tr.select("td").text());
				if (tr.select("td").size() <= 5 || tr.select("td").text().contains("Symbol")
			...
			//	System.out.println("tr-->:"+tr.select("td").text());
				...
				stockLive.setPercentchange(Math.abs(Double.parseDouble(NumberUtil.convertToNumber(td5.text()))));
			...
			for (Element tr : tr_turnover) {
				if (tr.text().contains("Symbol") || tr.text().contains("As of") || tr.text().contains("View All"))
					continue;
				...
				stockTurnover.setTurnOver(Double.parseDouble(NumberUtil.decimalFormatter.format(Double.parseDouble(NumberUtil.convertToNumber(td1.text())))));
				stockTurnover.setLtp(Double.parseDouble(NumberUtil.convertToNumber(td2.text())));
				stockTurnovers.add(stockTurnover);
				//System.out.println("turn:"+stockTurnover);
			}
